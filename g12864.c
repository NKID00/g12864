...
/**
 * @brief Write initialize settings.
 */
void g_sc_write_setup(struct g_sc *sc);

/**
 * @brief write the address of graphics memory. (the unit of x is 16-bit word)
 */
...
/* TODO: Did not check the return value of malloc, etc. */
...
struct g_sc
{
    struct g_fb *fb, *current; // double buffering
...
void g_sc_write_setup(struct g_sc *sc)
{
    // use 8 bits MPU interface and basic instructions set
    g_sc_write_byte_command(sc, 0x30 /* 0b0011_0000 */);

    // clear character display memory
